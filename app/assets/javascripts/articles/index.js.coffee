jQuery ->
  ...
  $(document).on 'click', '.comment-button', (e) ->
    ...
    $.post "/articles/#{article_id}/comments.json", $("#article_#{article_id}").serialize(), (data) ->
      console.log "/articles/#{article_id}/comments.json"
      ...
      $("#article_#{article_id}").parent('.comments-wrapper').siblings('.comments').prepend(html)
      $('.new').fadeIn().removeClass('new').animate({ backgroundColor: 'yellow' }, 'slow').animate({ backgroundColor: 'transparent'  }, 'slow')

  $(document).on 'click', '.article-liked-false', (e) ->
    console.log '+ 1'
    article_id = $(e.target).attr('data-article-id')
